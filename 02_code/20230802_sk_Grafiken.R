...
kipp_graph<-ggplot(kipppunkt, aes(x=Jahr, y=Value, group=Wert))+
  ...
  geom_text(data = label,
            aes(family="AOK Buenos Aires", x=Jahr, y=Value, label =label), size = 7, inherit.aes = FALSE)+
...
barplot<-ggplot(balken, aes(x = Jahr, y = Value, fill = Wert))+
  ...
  geom_text(aes(family = "AOK Buenos Aires",
                x= Jahr, y= Value, label=format(Value, decimal.mark = ","), group = Wert), 
            position=position_stack(vjust=0.5, reverse=TRUE), color = "white", size = 6) +
